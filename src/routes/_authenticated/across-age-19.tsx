...
import { getBackdrop } from "@/lib/space-backdrop";
...
//
// PAISAGEM FIXA: o jogo ocupa a tela inteira e fica sempre
// deitado. Em celular em pé, o CSS gira 90° — não depende de
// travar a orientação, que o iOS não permite.
// =========================================================
...
function GameCanvas({ pilotAvatarUrl, shipImageUrl, shipKey, pilotName, startLevel, destinationId, destinationFact, upgradeLevels, onResult, onWaveCleared }: GameCanvasProps) {
  ...
  // PAISAGEM FIXA: o jogo cobre a tela inteira enquanto está aberto,
  // então a página por baixo não pode rolar junto.
  useEffect(() => {
    ...
    try {
      if (!document.fullscreenElement) {
        ...
        try {
          ...
          // o CSS de paisagem fixa já deixa a tela deitada mesmo assim.
        }
      } else {
  ...
  useEffect(() => {
    ...
    // cenário pintado do destino — sempre existe, com ou sem foto.
    // É o que fecha as 45 fases: nenhuma fica com fundo vazio, e se um
    // hotlink da NASA/ESO cair, a fase continua com cenário.
    ...
    function resize() {
      ...
      // repinta o cenário no tamanho novo (troca de fase, girar o celular,
      // entrar/sair de tela cheia). O módulo tem cache, então é barato.
      backdrop = getBackdrop(theme, W, H);
    }
    ...
    function drawShip(c: CanvasRenderingContext2D, thrusting: boolean) {
      ...
      // naves redondas/simétricas (moto, anelar, OVNI, hélice) são desenhadas
      // girando de verdade, em 360°. Naves "de lado" (carro, jato) usam o
      // MESMO ângulo físico por trás (mesmo giro, mesmo motor na traseira),
      // mas na hora de desenhar a gente limita a rotação visual a ±90° e
    ...
    function draw() {
      ...
      // ===== CENÁRIO PROCEDURAL (todas as 45 fases) =====
      // Desliza junto com a câmera. Quando a fase tem foto, ela entra por
      // cima e o cenário fica como profundidade extra por baixo.
      if (backdrop) {
        const pX = WORLD_W > W ? camX / (WORLD_W - W) : 0.5;
        const pY = WORLD_H > H ? camY / (WORLD_H - H) : 0.5;
        ctx.drawImage(backdrop, -(backdrop.width - W) * pX, -(backdrop.height - H) * pY);
      }
    ...
    function update(dt: number) {
      ...
      //
      // A física é a mesma pra todas as naves — sempre gira de verdade e o
      // motor sempre empurra na direção do bico. A diferença entre nave
      // "redonda" e nave "de lado" é só visual (em drawShip): a de lado
      // espelha em vez de continuar girando quando passaria de cabeça pra
      // baixo, mas o ângulo físico por trás continua o mesmo pras duas.
    ...
    // redimensiona e repinta quando a tela muda de forma — inclusive ao
    // girar o celular, que na paisagem fixa troca largura por altura.
    function refresh() { resize(); draw(); }
    function onOrientation() { window.setTimeout(refresh, 260); }
...
.across-age-root{ --void:#0a0e27; --void-deep:#050714; --gold:#e8b34a; --gold-dim:#8a6a2a; --teal:#3ddbc9; --danger:#ff5c5c; --ink:#eef0ff; --ink-dim:#9aa0c8;
  /* PAISAGEM FIXA — o jogo toma a tela inteira e fica sempre deitado */
  position:fixed; inset:0; z-index:9990; margin:0; border-radius:0; overflow:hidden;
...
/* celular em pé: gira 90° por CSS e continua deitado, sem depender de travar
   a orientação (o iOS não permite travar) */
...
/* tela cheia de verdade: esconde a barra do navegador */
.across-age-root:fullscreen{ width:100vw !important; height:100vh !important; border-radius:0; }
.across-age-root:-webkit-full-screen{ width:100vw !important; height:100vh !important; border-radius:0; }
