...
function GameCanvas({ pilotAvatarUrl, shipImageUrl, shipKey, pilotName, startLevel, destinationId, destinationFact, upgradeLevels, onResult, onWaveCleared }: GameCanvasProps) {
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      // ===== FUNDO COM FOTO — desenhado em coordenadas de TELA, por isso
      // fica AQUI, antes do translate da câmera. "cover" enche a tela
      // inteira mantendo a proporção (corta o excesso, nunca estica) e
      // desliza com a câmera (parallax); "distant" desenha o astro pequeno
      // no canto do céu (discos de planeta e estrelas). =====
      if (bgImgReady) {
