...
// ============================================================================
//
// A miniatura da loja usa EXATAMENTE a mesma regra que o jogo: o campo
// `flipX` de cada nave em src/lib/ship-stats.ts. Assim a nave aparece na
// vitrine já do jeito que vai voar — e existe UMA lista só pra manter.
//
// Antes havia uma lista NEEDS_MIRROR aqui, separada do flipX do
// ship-stats.ts. As duas se contradiziam e era por isso que a nave da
// vitrine e a nave em voo apareciam viradas pra lados diferentes.
//
// Pra corrigir a orientação de uma nave, mexa SÓ no ship-stats.ts:
//   noseAngleDeg: 0   → nave de perfil, bico deve ficar pra DIREITA.
//                       Arte com bico pra esquerda? flipX: true.
//   noseAngleDeg: -90 → nave oval/disco/cabine, propulsão na BASE.
//                       Já está certa — normalmente sem flipX.
// ============================================================================

function shipThumbTransform(id: string) {
  return getShipStats(id).flipX ? "scale(1.08) scaleX(-1)" : "scale(1.08)";
}
